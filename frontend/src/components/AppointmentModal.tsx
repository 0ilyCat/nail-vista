  ...
  const styleOptions = merchantStyles.map(s => ({
    label: (
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span>{s.name}</span>
        <span style={{ color: '#c77986', fontWeight: 600, fontSize: 13 }}>¥{s.price}</span>
      </div>
    ),
    value: s.id,
  }));
  ...
  return (
    ...
    >
      ...
      {styleName && !selectedStyleId && (
        <div style={{ marginBottom: 16, padding: '10px 14px', background: '#fdf2f4', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
          <Tag color="#c77986" style={{ margin: 0 }}>推荐款式</Tag>
            ...
            {slots.map(slot => (
              <div
                ...
                style={{
                  padding: '8px 14px',
                  borderRadius: 20,
                  border: selectedSlot === slot.start ? '2px solid #c77986' : '1px solid #e8e8e8',
                  background: selectedSlot === slot.start ? '#fdf2f4' : '#fff',
