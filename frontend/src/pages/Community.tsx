  ...
  const renderCard = (p: any) => (
    <div
      ...
      style={{
        ...
        border: '1px solid #e8ede6', cursor: 'pointer',
    ...
    >
      <div style={{ position: 'relative', paddingTop: '100%', background: '#eef5eb', overflow: 'hidden' }}>
        <img
        ...
        {(!p.image_url && !p.images?.length) && (
          <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%',
            display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a0b89d', fontSize: 36 }}>💅</div>
        )}
        {(p.images?.length || 0) > 1 && (
          <div style={{
      ...
      <div style={{ padding: '10px 12px 12px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
            <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#eef5eb',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 11, color: '#7d9d7a', flexShrink: 0, fontWeight: 600 }}>{p.author_name?.[0] || <UserOutlined />}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            <span onClick={e => { e.stopPropagation(); onLike(p.id); }}
              style={{ fontSize: 12, cursor: 'pointer', color: p.is_liked ? '#7d9d7a' : '#bbb' }}>{p.is_liked ? <HeartFilled /> : <HeartOutlined />} {p.likes_count}</span>
            {p.style_price > 0 && <Tag color="rgba(199,121,134,0.12)" style={{ color: '#7d9d7a', border: 'none', fontWeight: 600, fontSize: 12, margin: 0 }}>¥{p.style_price}</Tag>}
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 0 80px' }}>
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
        <Col>
          <Title level={2} style={{ color: '#5a7a52', margin: 0 }}>灵感广场</Title>
        </Col>
        <Col>
          <Button type="primary" size="large" icon={<PlusOutlined />} onClick={openPublish}
            style={{
              ...
              fontWeight: 600, boxShadow: '0 2px 8px rgba(199,121,134,0.25)',
              border: 'none',
            }}>
            发布帖子
          </Button>
      ...
      <div style={{ marginBottom: 20 }}>
        {['recommend', 'latest'].map(t => (
          <Button key={t} type={tab === t ? 'primary' : 'default'}
            ...
            style={{ marginRight: 8, borderRadius: 20, borderColor: tab === t ? undefined : '#e8ede6', color: tab === t ? undefined : '#5a7a52' }}>
            {t === 'recommend' ? '🔥 推荐' : '🕐 最新'}
          </Button>
        ))}
      </div>
      ...
      {/* ══════════════════════ 发布弹窗 ══════════════════════ */}
      <Modal
        title={<span style={{ color: '#5a7a52', fontSize: 18, fontWeight: 600 }}>发布帖子</span>}
        open={publishOpen}
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          {/* 标题 */}
          <div>
            <label style={{ display: 'block', fontWeight: 600, marginBottom: 6, color: '#5a7a52', fontSize: 13 }}>
              标题 <span style={{ color: '#7d9d7a' }}>*</span>
            </label>
            <Input
              value={pubTitle}
          ...
          {/* 图片上传（多图） */}
          <div>
            <label style={{ display: 'block', fontWeight: 600, marginBottom: 8, color: '#5a7a52', fontSize: 13 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              ...
              {pubImages.length < 3 && (
                <Upload beforeUpload={onImageSelect} showUploadList={false} accept="image/*">
                  <div style={{
                    width: 100, height: 100, border: '2px dashed #e8ede6', borderRadius: 8,
                    display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                    cursor: 'pointer', color: '#7d9d7a', background: '#eef5eb',
                  }}>
        ...
        {detailLoading ? (
          ...
          <div>
            {detailPost.image_url && (
              <div style={{ background: '#eef5eb', display: 'flex', justifyContent: 'center' }}>
            ...
            <div style={{ padding: '20px 24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#eef5eb',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 16, fontWeight: 700, color: '#7d9d7a' }}>{detailPost.author_name?.[0] || <UserOutlined />}</div>
                  <div>
                ...
                <Button icon={detailPost.is_liked ? <HeartFilled style={{ color: '#7d9d7a' }} /> : <HeartOutlined />}
              ...
              {detailPost.style && (
                <div style={{ background: '#eef5eb', borderRadius: 10, padding: '12px 16px',
              ...
              {detailPost.related_posts?.length > 0 && (
                <div style={{ marginTop: 20 }}>
                  <div style={{ fontWeight: 600, fontSize: 14, color: '#5a7a52', marginBottom: 10 }}>相关推荐</div>
                  <Row gutter={[8, 8]}>
                    {detailPost.related_posts.map((rp: any) => (
                      <Col span={8} key={rp.id}>
                        <div onClick={() => openDetail(rp)} style={{ borderRadius: 8, overflow: 'hidden', cursor: 'pointer', border: '1px solid #e8ede6' }}>
                          <div style={{ height: 100, overflow: 'hidden', background: '#eef5eb' }}>
